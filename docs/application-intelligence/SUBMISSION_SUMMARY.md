...
This framework was built with an **agentic development pipeline**: purpose-built AI subagents reconnoitered the live application and generated the test suite under an orchestrator, with validated, human-reviewed hand-offs at every stage. The result combines AI-assisted velocity with senior-architect oversight — not a hand-written test suite, but the verified output of an automated build pipeline.

### Pipeline Stages

1. **Recon & Scaffold — `playwright-cli-explorer`**
   Drove the live app with `@playwright/cli` to produce `.playwright-cli/scaffold/emi-calculator.scaffold.md` — a verified inventory of pages, components, and locators (logical name → selector → element type), user flows and expected outcomes, including documented divergences from the test plan (text inputs instead of sliders, Tab-commit recalculation, third-party ad/analytics routes to block).

2. **Code Generation — `playwright-test-automator`**
   Generated the full suite (config, page objects, fixtures, test data, specs, CI workflow) from the scaffold, guided by the `playwright-best-practices` skill.

3. **Orchestration & Compliance Gate — `playwright-orchestrator`**
   Verified hand-offs, ran lint/typecheck, and enforced the two highest-severity anti-pattern bans: no hard waits (`waitForTimeout`/`sleep`) and no raw inline selector literals (all locators centralized in the selector registry).

### Evidence (in-repo)

- `.opencode/agents/` — agent definitions (explorer / orchestrator / automator)
- `.opencode/skills/playwright-best-practices/` — coding-standard skill
- `.playwright-cli/scaffold/emi-calculator.scaffold.md` — verified UI scaffold artifact
- `.playwright-cli/page-*.yml` — persisted live-page ARIA snapshots
...
- Purpose-built AI subagents (explorer / orchestrator / automator) built the suite
- Live-app reconnaissance with verified locators — no guesswork selectors
- Best-practices skill enforced web-first, no-hard-wait generation rules
- Orchestrator compliance gate before delivery
- Every hand-off validated and human-reviewed; evidence in-repo
...
- **Cutting-edge agentic development:**
  - Framework generated by an orchestrator-driven pipeline of AI subagents
  - Live-app recon with verified locators and a compliance gate
  - Best-practices skill governance for deterministic, maintainable tests
